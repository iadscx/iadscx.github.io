...
import { ChevronDown, Sparkles, Linkedin, MessageCircle , ArrowDown} from 'lucide-react';
const Hero = ({theme}) => {
  const scrollToAbout = () => {
  ...
  const goToProjects = () => {
    // Utiliza el método de scroll con desplazamiento suave hacia la sección de proyectos
      ...
      <div className="relative z-10 text-center max-w-5xl mx-auto">
        ...
        duration: 0.8
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass-effect">
            ...
            <span className="text-sm font-medium text-[rgb(var(--text-primary))]">"Los datos y el petróleo son meras promesas. Solo la Ingeniería crea las vías necesarias para convertir su potencial en la riqueza real.”</span>
          </div>
        </motion.div>
        ...
        duration: 0.8,
        delay: 0.4
      }} className="text-5xl md:text-7xl lg:text-7xl font-extrabold mb-4">
          <span className="text-gradient">Especialista en ingenieria de datos</span>
        </motion.h1>
        ...
        delay: 0.8
          ...
          <button onClick={openWhatsApp} className="px-8 py-4 bg-white/10 border border-white/20 text-white rounded-lg font-semibold text-lg hover:bg-white/20 transition-all duration-300 transform hover:scale-105 flex items-center gap-2">
          ...
          <button onClick={goToProjects} className="px-8 py-4 bg-white/10 border border-white/20 text-white rounded-lg font-semibold text-lg hover:bg-white/20 transition-all duration-300 transform hover:scale-105 flex items-center gap-2">
